...
%In this work, we need the galaxy sample well-studied in a wide wavelength range since we need to investigate how the low-frequency emission is related to galaxy properties.
%Therefore, we choose galaxies from the Herschel Reference Survey (HRS) catalog \citep{Boselli+2010}.
%The details and series papers about HRS galaxies are compiled on the HRS website (\url{https://hedam.lam.fr/HRS/}).
...
The selected galaxies located in the sky region between $10\msu{h}17\msu{m}< \mr{R.A.}(2000) < 14\msu{h}43\msu{m}$ and $-6^{\circ} < \mr{decl.} < 60^{\circ}$ (Figure~\ref{fig:Boselli2010_figure1})
