...
\usepackage[inner=2.5cm,outer=2.5cm,top=3.5cm=bottom=2cm]{geometry}
% For printing use:
...
%	\layout
	\begin{titlepage}
        ...
        \vspace{1.5cm}
 		\textbf{Supervisor:} Dr.\ Tsutomu\ T.\ Takeuchi\\
        \textbf{Co-Supervisors:} Dr.\ Barbara Catinella, Dr.\ Luca Cortese and Dr.\ O.\ Ivy Wong\\
	\end{titlepage}
	%\maketitle
	\frontmatter
%	\linenumbers
...
\newgeometry{inner=2.5cm,outer=2.5cm,top=1.5cm=bottom=2cm}
	\begin{abstract}
        Star formation rate (SFR) is one of the most fundamental quantities of a galaxy.
        Recombination lines, far-ultraviolet, and infrared (IR) luminosities, for example, are commonly used observational indicators of the SFR \citep[e.g.,][]{Kennicutt2012}.
        In addition, some references \citep[e.g.,][]{Condon1992a} have suggested the use of low-frequency emission (at GHz or lower) as a possibly better-performing SFR indicator compared to the traditional indicators, since it is insensitive to dust extinction.
        ...
        Low-frequency emission is often approximated as a power-law, and recent spatially resolved radio observations show that the spectral index shows a large variety depending on the location in a galaxy \citep{Kapinska2017a, For2018a}.
        ...
        We select galaxies from the Herschel Reference Survey (HRS) catalog \citep{Boselli2010}, which is assumed to be representative of local galaxies.
        We identify 18 star-forming galaxies with high-quality radio data from the GaLactic Extragalactic All-sky MWA (GLEAM) survey catalog \citep{Hurley-Walker2017a}.
        ...
        Firstly, we find that a single power-law fitting is valid for modeling the relation of radio with IR luminosities from MWA frequencies to $1500\,\mathrm{MHz}$.
...
\begin{figure}[htbp]
    ...
    \caption[Galaxy images (6/18 used for the analysis)]{\label{fig:galaxyimages}
        The SDSS \citep{Abolfathi2018} i-band images with radio contours from the GLEAM survey at $170\mbox{--}231\MHz$ (solid green lines).
...
\begin{figure}[htbp]
    ...
    \caption[Galaxy images (9/15 not used for the analysis)]{\label{fig:galaxyimages_notselected}
        Not used for the analysis labeled in Section~\ref{sec:reducegalaxysamples} although they have a radio counterpart in the GLEAM catalog by the cross-matching.
    }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption[Galaxy images (6/15 not used for the analysis)]{\label{fig:galaxyimages_notselected2}
        Continued.
    }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption[Galaxy images (suspicious matching)]{\label{fig:galaxyimages_suspicious}
        Suspicious matching galaxies in Section~\ref{sec:crossmatching}
    }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption[Galaxy images (no matching)]{\label{fig:galaxyimages_nomatch}
        No radio counterpart gaiaxies from the GLEAM catalog in Section~\ref{sec:crossmatching}
    }
\end{figure}
