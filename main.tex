...
%	\include{Declaration/declaration}
	...
	\begin{abstract}
        Star Formation Rate (SFR) is one of the most fundamental and important values for understanding galaxy evolution.
        So far, we measure it with various indicators, such as recombination lines, Far-ultra violet, and IR luminosities \citep{Kennicutt2012}.
        Recently, the low-frequency emission ($\mr{GHz}$ and less than that) is attracting attention as an SFR indicator.
        ...
        However, we still do not understand the spectral energy distribution at low frequencies, like optical to IR\@.
        Recent studies about the spatially resolved observation show us that the spectral index at low frequencies differs depends on location in a star-forming galaxy \citep{Kapinska2017a, For2018a}.
        This result suggests that there is no guarantee for radio emission from star-forming galaxies to have a simple frequency dependence.
        Now, we need to investigate how the relation between radio emission and star-formation (SF) activity varies across the low frequencies.

        In this study, we investigate the frequency dependence of the global relation between the low-frequency radio and infrared luminosities represented for the SF activity in nearby star-forming galaxies.
        We select galaxies from the Herschel Reference Survey catalog \citep{Boselli2010}, which is supposed to be well-represented for a local galaxy sample.
        We confirm 18 star-forming galaxies are appropriate for our study because they have high-quality radio data in the GaLactic Extragalactic All-sky MWA survey catalog \citep{Hurley-Walker2017a}.
        The radio sources compiled by the catalog have 20 narrow bands at $72 - 231\MHz$, and they are suitable for a more accurate examination of the frequency dependence.

        Our results are as follows:
        Firstly, we find that a single power-law fitting is valid for grasping how the relation of the radio with IR luminosities varies across MWA frequencies to $1.5\GHz$.
        This result suggests that the variation of the spectral index on the local scale does not affect the global relation.
        Secondly, we find that SFR calculated from the low-frequency radio emission is consistent with it from other indicators.
        In this study, we calculate SFR in two ways.
        The first one is to use the individual calibration using the fitting result to calculate it for each galaxy.
        ...
        While the former calibration gives us a consistent result with $\sim$ a $20\%$ scatter, the latter one does with two times more significant scatter than the former one.
        The more considerable scatter in the latter case would attribute to the intrinsic uncertainty of the calibration.
        After all, we propose to use the individual spectral energy distribution for calculating the radio SFR with less uncertainty.
